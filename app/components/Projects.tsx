...
const ACCENTS = [
  { dot: "bg-blue-400",    label: "text-blue-400",    shadow: "hover:accent-glow-blue"    },
  { dot: "bg-purple-400",  label: "text-purple-400",  shadow: "hover:accent-glow-purple"  },
  { dot: "bg-cyan-400",    label: "text-cyan-400",    shadow: "hover:accent-glow-cyan"    },
];
              ...
              <div className="flex flex-col gap-6 relative z-10">
                <div className="flex flex-col gap-3">
                  ...
                  <h3 className="text-2xl font-bold text-white tracking-tight leading-tight flex items-center gap-3">
                    {project.title}
                    <ArrowUpRight className="w-5 h-5 text-zinc-500 opacity-0 -translate-x-2 translate-y-2 group-hover:opacity-100 group-hover:text-white group-hover:translate-x-0 group-hover:-translate-y-0 transition-all duration-300" />
                  </h3>
                </div>

                <div className="flex flex-wrap gap-2">
                  {project.tech.map((t: string) => (
                    <span key={t} className="text-[11px] font-medium px-3 py-1.5 rounded-full bg-blue-950/30 text-zinc-300 border border-blue-500/10">
                      {t}
                    </span>
                  ))}
                </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-12">
            ...
            >
              ...
              <div className="flex flex-col gap-4 relative z-10 pr-10">
                ...
                <h3 className="text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-white to-zinc-400 tracking-tight leading-tight">
              ...
              <div className="flex flex-wrap gap-2 relative z-10">
                {selectedProject.tech.map((t: string) => (
                  <span key={t} className="text-xs font-medium px-3.5 py-1.5 rounded-full bg-blue-950/20 text-blue-100 border border-blue-500/20">
                    {t}
                  </span>
                ))}
              </div>
                  ...
                  <div className="flex flex-col gap-4 mt-2">
                    {selectedProject.bullets.map((b: string, idx: number) => (
                      <div key={idx} className="flex gap-4 group/bullet transition-transform hover:translate-x-1 duration-300">
                        ...
                        <p className="text-sm sm:text-[15px] text-zinc-400 group-hover/bullet:text-zinc-200 leading-[1.8] transition-colors">
                          {b}
                        </p>
                      </div>
                    ))}
